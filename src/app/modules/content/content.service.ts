...
const createContent = async (payload: IContent, user: JwtPayload) => {
  if (!(payload?.contentFiles || payload?.contentLinks)) {
...
const deleteContent = async (contentId: string, user: JwtPayload) => {
  const isContentExists = await Content.findById(contentId).populate(
    "classroom"
  );
  if (!isContentExists) {
  ...
  if (
    (
      isContentExists.classroom as unknown as IClassroom
    )?.faculty?.toString() !== user._id
  ) {
